...
import { buildCompanyInvoiceEmail } from '../_lib/adminEmailTemplates';
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const lead = {
      ...
      quotedAmountAed: amount,
      companyInvoiceNumber: invoiceNumber,
    };
    const { subject, htmlBody } = buildCompanyInvoiceEmail(lead);
    ...
    console.error('ADMIN_SEND_INVOICE_HANDLER_ERROR', err);
    sendJson(res, 500, {
      ok: false,
      error: 'INTERNAL_ERROR',
      message: 'Unable to send invoice right now.',
    });
  }
}
